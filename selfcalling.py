...
def single_self_calling():
    node_open = {"id" : "node_open", "task_type" : "class", "task_identifier" : "selfcalling.ProxyOpenManyData"}
    node_open_mask = {"id" : "node_open_mask", "task_type" : "class", "task_identifier" : "selfcalling.ProxyOpenDataMask"}
    node_config = {"id" : "node_config", "task_type" : "class", "task_identifier" : "ewoksxrpd.tasks.pyfaiconfig.PyFaiConfig"}
    node_print = {"id" : "node_print", "task_type" : "class", "task_identifier" : "selfcalling.ProxyPrint"}
...
class ProxyOpenManyData2(
    Task,
    input_names=["list_filenames", "energy", "geometry", "detector", "mask"],
    optional_input_names=["index"],
    output_names=["image", "filename_out", "list_filenames", "index", "repeat", "energy", "geometry", "detector", "mask"],
):
    def run(self):
        self.outputs.list_filenames = self.inputs.list_filenames
        self.outputs.energy = self.inputs.energy
        self.outputs.geometry = self.inputs.geometry
        ...
        filename_out = str(filename).replace(".edf", "_1d.dat")
        self.outputs.filename_out = filename_out
        self.outputs.index = index + 1

class ProxyPrint2(
    Task,
    input_names=["image", "geometry", "energy", "detector", "mask"],
    output_names=["image", "geometry", "energy", "detector", "mask"],
):
    def run(self):
        # print("I'm at ProxyPrint")
        image = self.inputs.image
...
def self_calling_2():
    node_open = {"id" : "node_open", "task_type" : "class", "task_identifier" : "selfcalling.ProxyOpenManyData2"}
    node_open_mask = {"id" : "node_open_mask", "task_type" : "class", "task_identifier" : "selfcalling.ProxyOpenDataMask"}
    node_config = {"id" : "node_config", "task_type" : "class", "task_identifier" : "ewoksxrpd.tasks.pyfaiconfig.PyFaiConfig"}
    node_print = {"id" : "node_print", "task_type" : "class", "task_identifier" : "selfcalling.ProxyPrint2"}
    node_integrate = {"id" : "node_integrate", "task_type" : "class", "task_identifier" : "ewoksxrpd.tasks.integrate.Integrate1D"}
    node_save = {"id" : "node_save", "task_type" : "class", "task_identifier" : "ewoksxrpd.tasks.ascii.SaveAsciiPattern1D"}
    ...
    convert_graph(graph, "self_calling_2.json")
    chunk_files = [str(item) for item in Path("edf_data/").glob("*.edf")]
    poni = "/users/edgar1993a/work/ewoks_parallel/lab6.poni"
    mask = "/users/edgar1993a/work/ewoks_parallel/lab6_mask.edf"
    ...
    execute_graph(
        ...
        inputs=[
            ...
            {"name" : "edf_filename", "value" : mask, "id" : "node_open_mask"},
            {"name" : "filename", "value" : poni, "id" : "node_config"},
            {"name" : "filename", "value" : "kkkkk.dat", "id" : "node_save"},
        ],
    )
